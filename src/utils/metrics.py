"""
Metrics computation module for next-location prediction.
Implements the exact metric specifications provided.
"""
...
def calculate_correct_total_prediction(logits, true_y):
    """
    Calculate top-k accuracies and other metrics.
    
    Args:
        logits: Model output logits (B, num_classes)
        true_y: True labels (B,)
        
    Returns:
        metrics array, true labels, top1 predictions
    """
    top1 = []
    result_ls = []
    for k in [1, 3, 5, 10]:
...
def get_performance_dict(return_dict):
    """
    Convert raw metrics to performance dictionary with percentages.
    
    Args:
        return_dict: Dictionary with raw metric counts
        
    Returns:
        Dictionary with computed percentages
    """
    perf = {
